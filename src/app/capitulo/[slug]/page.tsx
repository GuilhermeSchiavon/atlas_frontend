...
export default function CapituloPage({ params }: CapituloPageProps) {
  // Try to find chapter by ID first, then by slug
  const isNumeric = /^\d+$/.test(params.slug);
  const chapterId = isNumeric ? params.slug : null;
  
  const { chapter, isLoading: chapterLoading, error: chapterError } = useChapter(chapterId || params.slug);
